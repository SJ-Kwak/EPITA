...
void v(SDL_Renderer* renderer, int x, int y, double len, double a, int level)
{
    ...
    else if (level <= TOP_LEVEL)
    {
        ...
        SDL_RenderDrawLine(renderer, x, y2, x2, y);
        
        for(int i = 0; i < pow(2, i); i++){
            if(i % 2 != 0){
                v(renderer, x2, y2, RATIO * len, a + STEP_ANGLE, level);
            }
            else{
                v(renderer, x2, y2, RATIO * len, a + STEP_ANGLE, level);
            }
...
int main()
{
    ...
    // Creates a window.
    SDL_Window* window = SDL_CreateWindow("Static Fractal Canopy", 0, 0, INIT_WIDTH, 
INIT_HEIGHT,
